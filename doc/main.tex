...
\fixme{``has existed'' sounds like it doesn't anymore?}
...
section \ref{tabapi}), and should be trivial to expose to plugins.
...
plugin API. Furthermore, the \verb|tabs| and \verb|windows| modules contains
...
The tab API is grouped into various objects:
...
\end{tikzpicture}
\caption{Object diagram for tab API}
\end{figure}

\fixme{use filled diamond for composition}
...
However, for the most part, the tab API can be exposed unmodified to plugins and
already allow for a wide range of interactions with tabs.
...
\begin{itemize}
  ...
  \item The pytest project\footnote{\url{https://www.pytest.org/}} exposes a
    very simple and ``pythonic'' plugin API. All that's needed to create a
...
I'm thankful towards all qutebrowser contributors for their patience and
...
feedback on possible improvements. Her suggestions always were very welcome and
accurate.
...
\begin{itemize}
  ...
  \item In the future, after the extension API is open for third-party plugins,
    a v2.0.0 release with some other big changes (like dropping support for the
    older QtWebKit backend, as it's rarely updated upstream, even for security
    issues).
\end{itemize}
...
There's no external industry partner.
...
\fixme{Is it okay to move this before ``Project Schedule''?}
...
extension API for third-party extensions. Therefore, it's still possible to do

...
\usepackage{geometry}
%\geometry{top=2cm,bottom=2.5cm,includeheadfoot}
\setlist{parsep=10pt}
...
In this chapter \ref{ch:intro}, the background necessary to understand the
project and its goals is explained. Chapter \ref{ch:projectman} documents the
project methodology used and schedules the time available into various topics.
...
considered for the project, based on which a concept was designed in chapter
...
\fixme{How to cite y-approach?}

In order to take informed and sustainable design decisions, the
\emph{Y-Approach} \citep{yapproach} was evaluated. It proposes the following
...
board features were used for project management, together with
...
\begin{description}
  ...
  \item[Performance] Having code in extensions rather than the core part of a
...
previous work as an inspiration, in order to follow common standards and learn
...
\end{tikzpicture}
\caption{Class diagram for tab API (all relations are 1:1)}
\end{figure}
...
\verb|Tab| object is listed in figure \ref{fig:tabapi}.
...
using two properties: Them being either \emph{dynamically} or \emph{statically}
...
falling on mypy since it's widely used and backed (and used) by a more diverse
community than either pytype and pyre (which are mostly developed and used by
...
were missing stubs for PyQt, the main library being used by qutebrowser. While
...
\begin{itemize}
  \item Issues with the PyQt5-stubs project where type annotations where
